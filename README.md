...
| **Weight per position** | computed, reproduces the page column | The official holdings file has no weight column, so the updater reproduces the **Exposure Weight** column the fund pages render: the position value (market value, or notional exposure for futures and swaps) ÷ the fund total net assets in the same official file (the sum of its market values, including the `Net Other Assets (Liabilities)` line). Verified against the live pages: TQQQ NVDA `3.11%` and the Barclays swap `29.64%`, AGQ Silver DEC26 `77.04%`, NOBL BDX `1.73%`, IGHG Morgan Stanley `1.62%`. Cash and payable lines carry no weight on the pages and stay `—`. |
| **Gross vs net expense ratio** | both kept | ProShares publishes one ratio for most funds and a gross plus a net ratio for the geared funds (TQQQ `0.97%` gross / `0.82%` net). The feed headline (`ter`) and the catalog column are the net ratio; `meta.json` keeps `expenseRatio.gross` and `expenseRatio.net` and the Overview tab lists both. |
...
| `HISTORY_RANGE` | `max` | NAV-history window to publish: `max`, `20y`, `10y`, `5y`, or a number of months/days (`36m`, `90d`). |
